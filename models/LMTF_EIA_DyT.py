"""
LMTF-EIA 改进版本 v1: Dynamic Tanh 替代 LayerNorm

改进方向1: 保留对残差做注意力机制，但将 ResidualAttentionBlock 中的 2 个 LayerNorm 
替换为 Dynamic Tanh (DyT)，作为更灵活的归一化方式。
...
class DyT(nn.Module):
    """
    Dynamic Tanh 归一化层 —— 用可学习的 tanh 代替 LayerNorm。
    原理: 先用 alpha 缩放输入再过 tanh 压缩到 [-1, 1]，
          然后用 gamma/beta 做仿射变换恢复表达能力。
    """
    def __init__(self, C, init_alpha=0.5):
        super().__init__()
        # C: 特征维度（即 d_model）
        self.alpha = nn.Parameter(torch.ones(1) * init_alpha)   # 可学习缩放因子，标量
        self.gamma = nn.Parameter(torch.ones(C))                # 仿射变换的缩放，形状 (C,)
        self.beta = nn.Parameter(torch.zeros(C))                # 仿射变换的偏移，形状 (C,)

    def forward(self, x):
        # x: (..., C)  任意前导维度，最后一维是特征维度
        x = torch.tanh(self.alpha * x)       # 经 tanh 压缩到 [-1, 1]
        return self.gamma * x + self.beta     # 仿射变换还原表达力
...
# ======================== 残差注意力模块 (改进：用DyT替换LayerNorm) ========================

class ResidualAttentionBlock(nn.Module):
    """
    改进版: 用 Dynamic Tanh (DyT) 替代两个 LayerNorm
    保留架构不变，只改变归一化方式。
    """
...
# ======================== 主模型（结构保持不变，仅使用改进的ResidualAttentionBlock）========================
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # 4. 残差支路处理
        # 提取OT变量 (最后一个变量)
        ot_tensor = inp[:, :, -1:]  # (B, D, 1)
        
        # 事件强度检测
        event_intensity = self.event_detector(ot_tensor)  # (B, D, N)
        
        # 多层注意力处理（改进版：使用DyT替代LayerNorm）
        res_attn = residual
